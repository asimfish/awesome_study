...
\subtitle{awesome\_study 全库综述：32 篇精读 + 2026 趋势雷达}
...
\end{columns}
\vspace{6pt}
\textbf{四条解法路线（2026 全部成型）}：逐步分解（DPPO/DMPO）· 路径空间（FLAC/GSB-MDPO）· 噪声空间（DSRL/LP-DS）· 条件化监督（RECAP/AWR）
\end{frame}

\begin{frame}{仓库地图：六类 32 篇}
\small
\begin{tabular}{@{}p{0.24\textwidth}p{0.70\textwidth}@{}}
\toprule
\textbf{类别} & \textbf{篇目与定位} \\
\midrule
01 生成模型基础 (6) & DDPM · Score-SDE · FM · OT-CFM · MeanFlow · Diffusion Policy —— score $\to$ velocity $\to$ 平均速度场的一步化路线 \\
02 SB 理论 (4) & Léonard · DSB · GSBM · SB Foundations —— 数学正典 $\to$ 神经化 $\to$ 广义化 $\to$ 教科书 \\
03 Bridge 算法 (5) & I$^2$SB · DSBM · SF$^2$M · SB Flow · ASBM —— IPF $\to$ IMF $\to$ 免仿真 $\to$ 在线化 $\to$ 对抗少步 \\
04 RL 基础 (5) & PPO · SAC · MDPO · GRPO · AWR —— SB$\times$RL 的接口层 \\
05 生成式策略 $\times$ RL (8) & DT · Diffuser · Diffusion-QL · DPPO · MP1 · DMPO · OFP · DBPO —— 到原生一步竞赛 \\
06 SB $\times$ RL 前沿 (4) & \textbf{FLAC · GSB-MDPO · RSBM · BDGxRL —— 选题主战场} \\
\bottomrule
\end{tabular}
\vspace{4pt}

每篇配中文详细解读；12 篇前沿另配保版式中译 PDF。
\end{frame}
...
\textbf{产物}：32 篇中文详细解读 · 英文 PDF 32 篇 + 保版式中译 · 趋势报告（10 篇新雷达）· HTML PPT + 本 PDF。
\end{frame}
